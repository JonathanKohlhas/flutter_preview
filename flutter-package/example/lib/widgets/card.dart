...
class MusicCard extends StatelessWidget {
  ...
  Future<Color> getImagePalette(ImageProvider imageProvider) async {
    ...
    stream
        .addListener(ImageStreamListener((ImageInfo info, bool syncCall) => completer.complete()));
              ...
              child: BackdropFilter(
                ...
                child: FutureBuilder<Color>(
                    ...
                    builder: (context, snapshot) {
                      final color = snapshot.hasData ? snapshot.data!.withValues(alpha: 0.5) : null;
                      final brightness =
                          ThemeData.estimateBrightnessForColor(color ?? Colors.black);
...
class Resizable extends ResizablePreviewProvider with Previewer {
  ...
  Preview get preview {
    return Preview(
      mode: UpdateMode.hotReload,
      screenshotSettings: ScreenshotSettings(pixelRatio: 4, filename: 'test.png'),
      child: MusicCard(
        title: 'Blond',
        singer: 'Frank Ocean',
        image: PreviewImage.asset('preview/assets/cover1.jpg'),
...
class StaggeredCard extends StatelessWidget with Previewer {
  ...
  Widget build(BuildContext context) {
    ...
    return StaggeredGrid.count(
      ...
      children: [
        for (final (index, album) in list.indexed)
          StaggeredGridTile.count(
            mainAxisCellCount: 2,
            crossAxisCellCount: index.isOdd ? 2 : 4,
            child: MusicCard(
              radius: 12,
              onTap: () => {},
              title: album.title,
